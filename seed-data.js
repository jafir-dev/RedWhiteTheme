// Simple data seeding script for Vercel deployment
// This can be run using: node seed-data.js
...
console.log("🎰 Sample Prize Data for Fortune Wheel:");
console.log(JSON.stringify(samplePrizes, null, 2));
...
console.log("4. Run the following SQL to create sample prizes:");

console.log("\n-- Create Sample Prizes SQL:");
samplePrizes.forEach((prize, index) => {
  console.log(`INSERT INTO prizes (name, description, type, value, gold_grams, silver_grams, probability, color, is_active, created_at, updated_at) VALUES ('${prize.name}', '${prize.description}', '${prize.type}', ${prize.value}, ${prize.goldGrams}, ${prize.silverGrams}, ${prize.probability}, '${prize.color}', ${prize.isActive}, NOW(), NOW());`);
});

console.log("\n-- Create Sample Products SQL:");
sampleProducts.forEach((product, index) => {
  console.log(`INSERT INTO products (name, description, category, price_per_gram, weight_grams, total_price, image_url, in_stock, created_at, updated_at) VALUES ('${product.name}', '${product.description}', '${product.category}', ${product.pricePerGram}, ${product.weightGrams}, ${product.totalPrice}, ${product.imageUrl}, ${product.inStock}, NOW(), NOW());`);
});

console.log("\n✅ Data seeding instructions complete!");